...
export class OdooClient {
  ...
  private async rpc<T>(service: string, method: string, args: unknown[], model?: string) {
    const started = Date.now();
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), env.ODOO_TIMEOUT_MS);

    try {
      const response = await fetch(`${env.ODOO_URL.replace(/\/$/, "")}/jsonrpc`, {
        method: "POST",
      ...
      if (!response.ok) {
        throw new OdooError(`Odoo HTTP ${response.status}.`, "ODOO_HTTP_ERROR", { status: response.status });
      }
      ...
      logger.info({ method, model, durationMs: Date.now() - started }, "odoo request completed");
      return body.result as T;
    } catch (error) {
      const code = error instanceof OdooError ? error.code : "ODOO_NETWORK_ERROR";
      await this.logRequest(method, model, started, "error", code);
      logger.error({ error, method, model, durationMs: Date.now() - started }, "odoo request failed");
  ...
  private async logRequest(method: string, model: string | undefined, started: number, status: string, errorCode?: string) {
    try {
