...
import type { SearchResponse } from "@/components/inventory/types";
...
export function InventoryDashboard({ canEdit, username }: DashboardProps) {
  ...
  const [result, setResult] = useState<SearchResponse>({ rows: [], total: 0, source: "cache" });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const payload = (await response.json()) as SearchResponse | ErrorPayload;
        if (!response.ok) throw new Error((payload as ErrorPayload).error?.message ?? "Search failed.");
  ...
  const handleSave = async (updates: Array<{ productId: string; uaePriceAed: number | null; irPriceIrr: number | null }>) => {
    setIsSaving(true);
  ...
  const handleRefreshCache = async () => {
    ...
    try {
      const response = await fetch("/api/jobs/refresh-stale-cache", { method: "POST" });
      const payload = (await response.json()) as { refreshed?: number } & ErrorPayload;
      if (!response.ok) throw new Error(payload.error?.message ?? "Failed to refresh cache.");
      toast.success(`Refreshed ${payload.refreshed ?? 0} stale cache entries.`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to refresh cache.");
    } finally {
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-[0.24em] text-sky-700 dark:text-sky-300">Inventory pricing workspace</p>
            <h1 className="text-2xl font-semibold text-slate-950 dark:text-slate-100">Search products and maintain regional prices</h1>
            <p className="max-w-3xl text-sm text-slate-500 dark:text-slate-400">
              Search runs only when you press Enter or click Search. Results are cached locally and fall back cleanly if Odoo is unavailable.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            <div className="rounded-md border border-slate-200 px-3 py-2 dark:border-slate-700">
              Signed in as <span className="font-medium text-slate-800 dark:text-slate-200">{username}</span>
            </div>
            <div className="rounded-md border border-slate-200 px-3 py-2 dark:border-slate-700">
              {canEdit ? "Admin access" : "Viewer access"}
            </div>
            {canEdit ? (
              <Button variant="secondary" onClick={handleRefreshCache} disabled={refreshingCache}>
                <RefreshCw className={`size-4 ${refreshingCache ? "animate-spin" : ""}`} aria-hidden />
                Refresh stale cache
              </Button>
            ) : null}
          </div>
        ...
        <div className="mt-5">
          <SearchBar value={searchText} isSearching={isSearching} onChange={setSearchText} onSearch={handleSearch} />
        </div>
      </section>
      ...
      {submittedQuery ? (
        ...
        <section className="rounded-xl border border-dashed border-slate-300 bg-white/70 p-10 text-center text-sm text-slate-500 shadow-sm dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-400">
          Run a search by product name or barcode to load cached or live Odoo inventory data.
        </section>
      )}
    </div>
  );
